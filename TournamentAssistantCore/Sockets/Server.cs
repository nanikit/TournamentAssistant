                        ...
                        if (player.accumulatedBytes.Count >= PacketWrapper.packetHeaderSize)
                        {
                            ...
                            while (accumulatedBytes.Length >= PacketWrapper.packetHeaderSize && PacketWrapper.PotentiallyValidPacket(accumulatedBytes))
                            {
                                ...
                                try
                                {
                                    ...
                                    if (PacketReceived?.Invoke(player, readPacket.Payload).ConfigureAwait(false) is ConfiguredTaskAwaitable cta)
                                    {
                                        await cta;
                                    }
                                }
                ...
                while (webSocket.State == WebSocketState.Open)
                {
                    ...
                    if (receiveResult.MessageType == WebSocketMessageType.Binary)
                    {
                        ...
                        if (receiveResult.EndOfMessage)
                        {
                            ...
                            if (PacketReceived?.Invoke(player, readPacket).ConfigureAwait(false) is ConfiguredTaskAwaitable s)
                            {
                                await s;
                            }
